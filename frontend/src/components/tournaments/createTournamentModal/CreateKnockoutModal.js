...
export const CreateKnockoutModal = ({ user, token, onClose }) => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            formData.participants.forEach((player) => {
                uniqueFullNamesSet.add(player.full_name);
            });

            const selectedFullNames = Array.from(uniqueFullNamesSet).map(
                (fullName) => ({
                    full_name: fullName,
                })
            );
            ...
            const formDataWithUserId = {
                ...
                owner_id: user?.id,
                participants: selectedFullNames,
            };
