...
from models.requests import DirectorRequest, LinkToPlayerRequest
...
from services.players_service import get_player_by_user_id
...
def accept_link_player_request(request_id: int, current_user: users.User):
    ...
    if get_player_by_user_id(current_user.id):
...
def get_director_request_by_id(request_id: int):
    ...
    if result:
        result = result[0]
        return DirectorRequest.from_query_result(
            id=result[0],
            user_id=result[1],
            email=result[2],
            status=result[3],
...
def get_link_player_request_by_id(request_id: int):
    ...
    if result:
        result = result[0]
        return LinkToPlayerRequest.from_query_result(
            id=result[0],
            user_id=result[1],
            username=result[2],
